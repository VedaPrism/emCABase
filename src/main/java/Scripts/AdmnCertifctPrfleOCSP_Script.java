...
public class AdmnCertifctPrfleOCSP_Script extends Basefunctions {
	final static Logger log = Logger.getLogger(AdmnCertifctPrfleOCSP_Script.class);
	...
	public void certificateProfileOCSP(Map hm) throws Exception{	
		...
		pf_admnPfleMngntCertPrflePage c=new pf_admnPfleMngntCertPrflePage(w);
		if(scriptName.equalsIgnoreCase("certPrfleOCSP")) {
			et =  es.startTest("OCSP profile under CA");
			c.createOCSPPrfle(pType,pName,vDay,vHours,vMin,vSec,ICA,signAlg,OID,aEmail,aPswrd,shtName,scriptName);

		}
	}
	
	@Test(enabled=true,priority=3,groups={"SMK","REG"})
	public void exprtToExl() throws Exception{	
		pf_admnPfleMngntCertPrflePage c=new pf_admnPfleMngntCertPrflePage(w);
		et =  es.startTest("Certificate profile-ExprtExcel");
		c.exportExcel();
	}
