...
public class LoginAuditor_Script extends Basefunctions {
	final static Logger log = Logger.getLogger(LoginAuditor_Script.class);

	@Test(dataProvider= "valid_login",dataProviderClass=Dataproviders.dp_login.class,enabled=true,priority=1,groups={"SMK","REG"})
	public void LoginAuditor(Map hm) throws Exception{	
		
		//login
		String scriptName = hm.get("Script_Name").toString();
		et =  es.startTest("Login as Auditor");
		...
		if(scriptName.equalsIgnoreCase("ValidLoginAuditor")) {
